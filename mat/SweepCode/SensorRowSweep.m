...
fields = [linspace(0, 40, 21) linspace(38, 0, 20)]; % goes up to 40Oe
pause_time = .5; % time between changes
...
s.DurationInSeconds = .25; % .25s of acquisition
...
while true
	...
	for test_i = test_curr
		...
		fprintf('Testing %.3fA %.3fOe in coil.\n', test_i, test_i*ha);
	...
	for i = 1:7
		...
		mr(i) = relevant_ch(i,1:15)'\fields(1:15)';
	end
	calculated_mr{end+1} = mr;

	% ref calcs
	ref_mr(end+1) = row_data{end}(2, 1:15)'\fields(1:15)';
	ref_nom_r(end+1) = (row_data{end}(2, 1) + row_data{end}(2, end))/2;

	% NOTE TEMPORARY COMMENT FOR AUTOMATION
	% plotOnFig(f, fields, row_data, row_title, nominal_res, calculated_mr, ref_mr, ref_nom_r);
	...
	if length(savename)>2
		save(savename, 'calculated_mr', 'nominal_res', 'fields', 'row_data', 'row_title', 'ref_mr', 'ref_nom_r')
...
function plotOnFig(fig, fields, row_data, titles, nom_r, mr, ref_mr, ref_nom_r)
	...
	subplot(2, 4, 8);
	cla
	plot(fields, row_data{end}(2, :));
	ylabel('Ohms')
	xlabel('Oe')
	title(['Reference R_n=' num2str(ref_nom_r(end)/1000, '%.2f') 'k\Omega MR=' num2str(ref_mr(end)) '\Omega/Oe']);
