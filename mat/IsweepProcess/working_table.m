function working_table(savename, working, working_ref, figTitle)
	...
	f=figure('Visible', 'Off', 'Name', figTitle);
...
function show_plot(table, event, savename)
	...
	if j==0;
		data = T.row_data{i}(2,:);
		title_str = {[T.row_title{i} ' Ref \color{blue}R_n=' num2str(T.ref_nom_r(i)/1000, '%.2f') 'k\Omega \color{black}MR=' num2str(T.ref_mr(i)) '\Omega/Oe \color{blue}r^2=' num2str(T.ref_mr_r2(i), '%.3f')],['\color{black}MR(%)=' num2str(T.mr_perc_ref(i)*100, '%.5f') '%/Oe \color{blue}R_n(DC offset)=' num2str(T.r_off_max_ref(i)) '\Omega']};
	else
		data = T.row_data{i}(j*2-1,:);
		title_str = {[T.row_title{i} ' AI' num2str(j) ' \color{blue}R_n=' num2str(T.nominal_res{i}(j)/1000, '%.2f') 'k\Omega \color{black}MR=' num2str(T.calculated_mr{i}(j)) '\Omega/Oe \color{blue}r^2=' num2str(T.mr_r2{i}(j), '%.3f')],['\color{black}MR(%)=' num2str(T.mr_perc{i}(j)*100, '%.5f') '%/Oe \color{blue}R_n(DC offset)=' num2str(T.r_off_max{i}(j)) '\Omega']};
	end
