function process_raw(savename, lin_range); % !!!IMPORTANT!!! lin_range is OPTIONAL parameter
  ...
  for row = data
    ...
    for i = 1:2:14
      ...
      r_o(end+1) = range([curr_dat(1) curr_dat(midind) curr_dat(end)]);
    ...
    % mr percentage
    mr_perc_ref(end+1) = ref_mr(end) / ref_nom_r(end);

    % max nominal res offset
    r_off_max_ref(end+1) = range([curr_dat(1) curr_dat(midind) curr_dat(end)]);

  end

  save(savename, 'field_linear_range', 'nominal_res', 'ref_nom_r', 'calculated_mr', 'ref_mr', 'mr_r2', 'ref_mr_r2', 'mr_perc', 'mr_perc_ref', 'r_off_max', 'r_off_max_ref', '-append')
